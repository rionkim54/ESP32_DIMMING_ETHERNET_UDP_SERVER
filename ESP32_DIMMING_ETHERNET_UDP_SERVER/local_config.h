...
#define save_ip 64 // 96
#define save_ip2 88

/*
 * W5500 "hardware" MAC address.
 */
uint8_t eth_MAC[] = { 0x02, 0xF0, 0x0D, 0xBE, 0x80 | save_ip2, 0x01 };
 ...
 */
IPAddress eth_IP(192, 168, save_ip, save_ip2);		// *** CHANGE THIS to something relevant for YOUR LAN. ***
IPAddress eth_MASK(255, 255, 255, 0);		// Subnet mask.
IPAddress eth_DNS(192, 168, save_ip, 1);		// *** CHANGE THIS to match YOUR DNS server.           ***
IPAddress eth_GW(192, 168, save_ip, 1);		// *** CHANGE THIS to match YOUR Gateway (router).     ***

#define RESET_P	26				// Tie the Wiz820io/W5500 reset pin to ESP32 GPIO26 pin.

const uint16_t localPort = 8723;		// Local port for UDP packets.
